...
from utils.resources import techies,mine,start_icon,retur,cool_font,tools,ua_translation
from utils.map_converter import bob_map_to_map, map_to_bob_map, binary_to_map
from utils.tile import Tile
from utils.ui_utils import set_enabled_layout,get_auto_mines,get_color,read_config
from utils.constants import TILE_SIZE

class MapCreatorUI(QMainWindow):
    def __init__(self, theme:str, screen_w:int=1920, screen_h:int=1080) -> None:
        super().__init__()
        self.qtechies = QIcon(techies)
        self.qtechies_mine = QIcon(mine)
        ...
        self.setWindowIcon(QIcon(tools))
    ...
    def menu_UI(self) -> None:
        ...
        generate_new_button = QRadioButton(self.tr('Generate new map'))
        ...
        generate_new_layout0.addWidget(QLabel(self.tr('Width')))
        generate_new_layout0.addWidget(self.custom_width_line)

        generate_new_layout0.addWidget(QLabel(self.tr('Height')))
        generate_new_layout0.addWidget(self.custom_height_line)

        generate_new_layout0.addWidget(QLabel(self.tr('Mines')))
        generate_new_layout0.addWidget(self.custom_mines_line)

        generate_new_layout0.addWidget(QLabel(self.tr('Auto mines')))
        ...
        load_from_binary_button = QRadioButton(self.tr('Load map from binary string'))
        ...
        self.binary_box.setToolTip(self.tr('Enter a text image made of 0s and 1s'))
        binary_box_layout = self.create_offset_layout(self.binary_box)
        
        load_from_bobmap_button = QRadioButton(self.tr('Load map from bobmap'))
        load_from_bobmap_button.toggled.connect(lambda checked:self.set_start_bob() if checked else None)
        self.bob_map_box = QLineEdit()
        self.bob_map_box.setToolTip(self.tr('You can get bobmap by creating your map'))
        ...
        manual_start_layout.addWidget(manual_button, alignment=Qt.AlignmentFlag.AlignLeft|Qt.AlignmentFlag.AlignBottom)
        manual_start_layout.addWidget(start_button, alignment=Qt.AlignmentFlag.AlignRight|Qt.AlignmentFlag.AlignBottom)
        ...
        menu_layout.addLayout(manual_start_layout)
    ...
    def map_creator_ui(self):
        ...
        auto_n_checkbox = QCheckBox(text=self.tr('Auto numbers'))
    ...
    def update_tile_edit(self,tile_edit:TileEdit):
        tile = self.tile_edit_to_tile(tile_edit)
        base_color = tile_edit.base_color
        mine_color = '#4f4f4f'
        if self.theme == 'light':
            mine_color = '#999999'
        ...
        if tile.value == -1:
            ...
            tile_edit.setStyleSheet(tile_edit.default_style.replace(base_color, mine_color))
...
if __name__ == '__main__':
    ...
    if language_code == 'ua':
        translator.load(ua_translation)
        app.installTranslator(translator)

    screen_w,screen_h = app.primaryScreen().size().toTuple()
    window = MapCreatorUI(theme, screen_w, screen_h)
    window.show()
    app.exec()
